...
maximum_stress_per_object                   = event_per_object_from_population_mean(n,Smax1,Bsmax);
...
%NB resetting the landscape is necessary as current code doesn't model
%stress redistributing past pre-existing cracks via the Beam/substrate.

%to mimic the reduction of stress from the first round, Smax is reduced to
%half for the second round.

%to permit further designations at this reduced stress, precursor
%sensitivities are doubled.
...
precursor_sensitivity_matrix_p2 = precursor_sensitivity_matrix_p2.*2;

maximum_stress_per_object       = event_per_object_from_population_mean(n,Smax2,Bsmax);
...
% combine canonical majority and minority COs
